...
gulp.task('fileinclude', function() {
  gulp.src(['src/pages/uidemo/*.html'])
    ...
    .pipe(gulp.dest('dist/pages/uidemo/'));
});

//监听到所有与src/assets/less/*/*.less相匹配的文件的变化。
//一旦监测到变化，就会生成css并保存，然后重新加载网页
gulp.task('less', function() {
   gulp.src('src/assets/less/other-less/*.less')
      .pipe(watch("src/assets/less/other-less/*.less"))
      .pipe(less())
...
gulp.task('watch', function() {
    ...
    gulp.watch('src/assets/less/other-less/*.less', ['less']);
