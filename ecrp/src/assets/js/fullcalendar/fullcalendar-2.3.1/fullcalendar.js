...
DayGrid.mixin({
		...
		function emptyCellsUntil(endCol) { // goes from current `col` to `endCol`
			while (col < endCol) {
				...
				if (segsBelow.length) {
					...
					moreLink = _this.renderMoreLink(cell, segsBelow,levelLimit);
			...
			for (i = 0; i < levelSegs.length; i++) {
				...
				if (totalSegsBelow) { // do we need to replace this segment with one or many "more" links?
					...
					for (j = 0; j < colSegsBelow.length; j++) {
						...
						moreLink = this.renderMoreLink(cell, [ seg ].concat(segsBelow),levelLimit); // count seg as hidden too
						moreWrap = $('<div/>').append(moreLink);
	...
	renderMoreLink: function(cell, hiddenSegs,levelLimit) {
		var _this = this;
		var view = this.view;
		//console.log(cell.row,this.computeRowLevelLimit(3));
		return $('<a class="fc-more"/>')
			/*.text(
				this.getMoreLinkText(hiddenSegs.length)
			)*/
			.html(//显示更多时 更改样式
					' <span class=" badge badge-green" style="bottom: 0px;top:3px; " >总计'+(hiddenSegs.length+levelLimit-1)+'个</span>'
			)
